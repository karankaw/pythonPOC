https://pandas.pydata.org/


https://machinelearningmastery.com/handle-missing-data-python/
http://ipython.readthedocs.io/en/stable/
https://torbjornzetterlund.com/convert-a-ipython-notebook-into-a-python-file-via-command-line/
http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.replace.html
http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.fillna.html
https://pandas.pydata.org/pandas-docs/stable/generated/pandas.get_dummies.html
http://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.Imputer.html
http://scikit-learn.org/stable/auto_examples/plot_missing_values.html#sphx-glr-auto-examples-plot-missing-values-py
https://archive.ics.uci.edu/ml/machine-learning-databases/pima-indians-diabetes/pima-indians-diabetes.data
http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html#pandas.DataFrame

jupyter nbconvert --to python [notebook_name].ipynb

pandas.DataFrame.replace(regex, Value)
pandas.DataFrame.fillna( newValue, inplace=True)
pandas.get_dummies(dataset, columnsNameList)


imputer = sklearn.preprocessing.Imputer()
imputer.transform


